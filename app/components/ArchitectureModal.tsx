        ...
        >
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '32px 40px 24px', background: 'transparent', zIndex: 10 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
                {(product.logoHorizontal || product.icon) && (
                  <div style={{ 
                    background: (product.name.includes("SelectSys") || product.name.includes("mercadinhosys")) ? '#ffffff' : 'transparent',
                    padding: (product.name.includes("SelectSys") || product.name.includes("mercadinhosys")) ? '12px 24px' : '0', 
                    borderRadius: '16px', 
                    boxShadow: (product.name.includes("SelectSys") || product.name.includes("mercadinhosys")) 
                      ? '0 10px 30px rgba(0,0,0,0.4), inset 0 0 0 1px rgba(255,255,255,1)' 
                      : 'none',
                    display: 'flex', alignItems: 'center', justifyContent: 'center'
                  }}>
                    <img src={`${product.logoHorizontal || product.icon}?v=17`} alt={product.name} style={{ height: (product.name.includes("SelectSys") || product.name.includes("mercadinhosys")) ? '44px' : '56px', width: 'auto', objectFit: 'contain', filter: (product.name.includes("SelectSys") || product.name.includes("mercadinhosys")) ? 'none' : 'drop-shadow(0 4px 8px rgba(0,0,0,0.6))' }} />
                  </div>
                )}
                <div>
            ...
            <div style={{ padding: '24px 40px', background: 'rgba(0,0,0,0.5)', borderTop: '1px solid rgba(255,255,255,0.05)', boxShadow: 'inset 0 10px 20px rgba(0,0,0,0.2)', display: 'flex', gap: '12px', overflowX: 'auto', alignItems: 'center', zIndex: 10 }}>
                ...
                {product.tags && product.tags.map((tag: any, i: number) => (
