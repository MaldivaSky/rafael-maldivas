            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
              {(product.logoHorizontal || product.icon) && (
                <div style={{ 
                  ...
                  <img src={`${product.logoHorizontal || product.icon}?v=13`} alt={product.name} style={{ height: '52px', width: 'auto', objectFit: 'contain', filter: (product.name.includes("SelectSys") || product.name.includes("mercadinhosys")) ? 'drop-shadow(0 4px 6px rgba(0,0,0,0.05))' : 'drop-shadow(0 8px 16px rgba(0,0,0,0.5))' }} />
                </div>
              )}
              <div>
